...
const Game = struct {
    ...
    fn cursor(self: *Game) void {
        if (self.mouse.held(w4.MOUSE_LEFT)) {
            w4.color(0x33);
            w4.circle(self.mouse.x, self.mouse.y, 2);
        }
    }
    ...
    fn play(self: *Game) void {
        ...
        for (&self.dice) |*d| {
            ...
            if (self.button.released(0, w4.BUTTON_1) or self.mouse.released(w4.MOUSE_RIGHT) or self.rollBtn.clicked(&self.mouse, w4.MOUSE_LEFT)) {
                if (!d.h) d.startRoll();
            }
        ...
        w4.color(0x43);
        w4.text("ROLL", self.rollBtn.x + 8, self.rollBtn.y + 12);
